...
class Reservoir
    new: (x, y) =>
        ...
        for j=1, y+1
            insert @grid, {}
            if j == y+1 then insert @grid[#@grid], TILES.win for i=1, x+2
    ...
    nextStep: (p) =>
        x, y = unpack p
        return "WIN" if @grid[y+1][x] == TILES.win
